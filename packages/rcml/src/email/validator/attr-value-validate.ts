...
import { RCML_ATTR_VALIDATORS } from './attr-validators.js'
...
function visit(node: unknown, path: string, issues: EmailTemplateValidationIssue[]): void {
  ...
  if (typeof n.tagName === 'string') {
    ...
    const spec = RCML_SCHEMA_SPEC[tagName] as typeof RCML_SCHEMA_SPEC[RcmlTagName] | undefined
    if (spec && isPlainObject(n.attributes)) {
      for (const [name, value] of Object.entries(n.attributes)) {
        const attrSpec = spec.attrs[name] as typeof spec.attrs[string] | undefined
        if (!attrSpec) continue // unknown — AJV already reported it

        const zod = RCML_ATTR_VALIDATORS[attrSpec.validator]
