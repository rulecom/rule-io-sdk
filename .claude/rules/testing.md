...
## Validation Must Have Rejection Tests

When adding input validation (URL sanitization, field key checks, etc.),
always add tests that verify invalid input is **rejected**, not just that
valid input passes. A validation path without a rejection test is invisible
to the test suite and can regress silently.

```typescript
// WRONG - only tests happy path
it('creates head with valid URLs', () => { ... });

// CORRECT - also tests rejection
it('throws RuleConfigError for unsafe logoUrl', () => {
  expect(() =>
    createBrandHead({ ...style, logoUrl: 'javascript:alert(1)' })
